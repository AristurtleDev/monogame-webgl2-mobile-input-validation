...
class MonoGameWebHost {
    constructor(root) {
        ...
        this.accelerometerPermissionDiagnosticElement =
            this.config.accelerometerPermissionDiagnosticElementId == null
                ? null
                : document.getElementById(this.config.accelerometerPermissionDiagnosticElementId);
        this.canvas = null;
        this.runtime = null;
        ...
        this.accelerometerPermissionCanvasHandler = null;
    }
    ...
    readConfig(root) {
        ...
        return {
            ...
            accelerometerPermissionDiagnosticElementId:
                this.getOptionalConfigValue(dataset.accelerometerPermissionDiagnosticElementId)
                ?? this.getOptionalConfigValue(runtimeConfiguration.accelerometerPermissionDiagnosticElementId),
            accelerometerPermissionFromCanvas: this.getBooleanConfigValue(
                dataset.accelerometerPermissionFromCanvas,
                runtimeConfiguration.accelerometerPermissionFromCanvas,
                "accelerometer_permission_from_canvas")
        };
    }
    ...
    getBooleanConfigValue(datasetValue, runtimeConfigurationValue, settingName) {
        const value = datasetValue == null || datasetValue === ""
        ...
        throw new BrowserHostStartupError(
            HostStage.HostBootstrap,
            `${settingName}_invalid`,
            `Browser host setting '${settingName}' must be true or false.`);
    }
    ...
    resolveCanvas() {
        ...
        this.configureCanvasAccelerometerPermission();
    ...
    configureCanvasAccelerometerPermission() {
        if (!this.config.accelerometerPermissionFromCanvas) {
            return;
        }

        this.accelerometerPermissionCanvasHandler = async () => {
            if (this.runtime == null || !this.isAccelerometerPermissionRequired()) {
                return;
            }

            this.canvas.removeEventListener("click", this.accelerometerPermissionCanvasHandler);
            this.accelerometerPermissionCanvasHandler = null;
            await this.requestAccelerometerPermissionFromUserGestureAsync();
        };
        this.canvas.addEventListener("click", this.accelerometerPermissionCanvasHandler);
    }
    ...
    async requestAccelerometerPermissionAsync() {
        try {
            ...
            if (permission !== "granted") {
                this.reportAccelerometerPermissionDiagnostic(
                    `DeviceMotionEvent.requestPermission() returned '${permission}'.`);
                this.runtime.Module._MGP_Web_NotifyAccelerometerState(SensorState.NoPermissions);
                return false;
            }

            this.reportAccelerometerPermissionDiagnostic(
                "DeviceMotionEvent.requestPermission() returned 'granted'.");
            this.startAccelerometer();
            return true;
        }
        catch (error) {
    ...
    reportAccelerometerPermissionDiagnostic(message) {
        if (this.accelerometerPermissionDiagnosticElement == null) {
            return;
        }

        this.accelerometerPermissionDiagnosticElement.hidden = false;
        this.accelerometerPermissionDiagnosticElement.value = message;
    }
    ...
    getErrorDescription(error) {
        ...
        return error.stack == null || error.stack.length === 0
